...
export default function Home() {
  const { data: featured } = trpc.property.list.useQuery({
    status: "ACTIVE",
    page: 1,
    limit: 6,
  });
      ...
      <nav className="border-b bg-white/80 backdrop-blur sticky top-0 z-50">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t px-4 py-3 space-y-2 bg-white">
            <Link to="/ilanlar" className="block py-2" onClick={() => setMobileMenuOpen(false)}>İlanlar</Link>
            <Link to="/hakkimizda" className="block py-2" onClick={() => setMobileMenuOpen(false)}>Hakkımızda</Link>
            <Link to="/iletisim" className="block py-2" onClick={() => setMobileMenuOpen(false)}>İletişim</Link>
            <Link to="/login" className="block py-2" onClick={() => setMobileMenuOpen(false)}>Panel Girişi</Link>
          </div>
        )}
      </nav>
        ...
        <div className="max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="bg-white rounded-xl p-4 max-w-4xl mx-auto shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              <Link
                to={`/ilanlar?city=${searchCity}&type=${searchType}&listing=${searchListing}`}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card>
              <CardContent className="p-6 text-center space-y-3">
                ...
                <h3 className="font-semibold">Satış & Kiralama</h3>
                <p className="text-sm text-muted-foreground">
                  Tüm gayrimenkul türlerinde profesyonel satış ve kiralama danışmanlığı.
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-6 text-center space-y-3">
                <Maximize className="h-10 w-10 mx-auto text-primary" />
                <h3 className="font-semibold">Ekspertiz & Değerleme</h3>
                <p className="text-sm text-muted-foreground">
                  Uzman ekibimizle gayrimenkul değerleme ve ekspertiz hizmetleri.
                </p>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-6 text-center space-y-3">
                ...
                <h3 className="font-semibold">Yatırım Danışmanlığı</h3>
                <p className="text-sm text-muted-foreground">
                  Portföy yönetimi ve yatırım stratejileri ile kazançlı yatırımlar.
                </p>
              </CardContent>
            </Card>
          </div>
      ...
      <section className="py-16 max-w-7xl mx-auto px-4">
        <div className="bg-primary rounded-2xl p-8 md:p-12 text-white text-center">
          ...
          <p className="text-primary-foreground/80 mb-6 max-w-xl mx-auto">
            Gayrimenkul ihtiyaçlarınız için uzman danışmanlarımızdan randevu alın.
          </p>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Türkiye'nin lider gayrimenkul danışmanlık firmalarından biri olarak, satış, kiralama ve yatırım danışmanlığı hizmetleri sunuyoruz.
              </p>
            </div>
            <div>
